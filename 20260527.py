...
class RedTransporte:
    ...
    def dijkstra(self, origen, destino):
        """Calcula la ruta más rápida usando el algoritmo de Dijkstra."""
        if origen not in self.grafo or destino not in self.grafo:
            print("Error: Una o ambas estaciones no existen en la red.")
            return

        # Cola de prioridad (heap): almacena tuplas (tiempo_acumulado, estacion_actual)
        ...
        if tiempos_minimos[destino] == float('inf'):
            ...
            camino_formateado = " -> ".join(rutas[destino])
            print(f"\nRuta más rápida encontrada:")
            print(f"Recorrido: {camino_formateado}")
            print(f"Tiempo total: {tiempos_minimos[destino]} minutos")
...
def main():
    ...
    # Requisito: cargar la red al iniciar
    ...
    while True:
        ...
        try:
            if opcion == '1':
                ...
                # Usamos .title() para que "madrid" se convierta en "Madrid"
